    ...
    <div class="w-full px-4 md:px-0 md:mt-8 mb-16 text-gray-800 leading-normal">
        <section class="container px-4 mx-auto">
            ...
            <form action="/dashboard/books/{{ $book->f_id }}" method="POST"
                class="w-full flex flex-col mt-5 gap-4 max-w-6xl mx-auto" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="flex flex-col gap-2 text-white">
                    ...
                    <input type="number" name="stock" id="stock" class="bg-gray-950 py-2 px-3 rounded-md"
                        value="{{ count($book->detail) }}"
                        {{ count($book->detail) != count($book->detail->where('f_status', 'Tersedia')) ? 'disabled' : '' }}>
                    @error('stock')
                        <p class="text-red-500 px-3 text-xs">{{ $message }}</p>
                    @enderror
                    <p class="text-xs px-3">*Isi dengan angka</p>
                    <p class="text-xs px-3">*Jika Buku Sedang Dipinjam, Stok Tidak dapat diubah</p>
                </div>
                <div class="flex flex-col gap-2 text-white">
                    <h3 class="font-medium text-lg">Cover Buku</h3>
                    <div class="flex gap-2 items-start">
                        <input type="hidden" id="remove" name="remove" value="remove" disabled>
                        <label for="f_gambar"
                            class="bg-blue-600 w-fit px-2 py-2 cursor-pointer transition duration-150 hover:bg-blue-700 rounded-md">Upload
                            Foto</label>
                        <img src="{{ asset('/storage/' . $book->f_gambar) }}" alt="" id="preview_img"
                            class="w-32">
                        <button type="button"
                        ...
                        <input type="hidden" name="oldImage" value="{{ $book->f_gambar }}">
                    </div>
...
@section('script')
    ...
    <script>
        ...
        let loadFile = function(event) {
            ...
            const inputRemove = document.getElementById('remove').disabled = true;
        ...
        let removeImage = function(e) {
            ...
            const inputRemove = document.getElementById('remove').disabled = false;
